...
const EMPTY_TIER: PricingTier = { label: "", price: "", deposit: "", hours: "", description: "" };
...
export function PricingEditor({
  ...
  const [tiers, setTiers] = useState<PricingTier[]>(
    initialTiers.length > 0 ? initialTiers : [{ ...EMPTY_TIER }]
  );
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h2 className="text-cream text-lg font-light mb-1">Pricing</h2>
        <p className="text-muted text-sm">
          Define your pricing tiers. These are displayed on your public booking page.
        </p>
      </div>

      {tiers.map((tier, i) => (
        <div key={i} className="border border-border p-5 flex flex-col gap-4">
      ...
      {saved && <p className="text-green-400 text-xs">Pricing saved.</p>}
